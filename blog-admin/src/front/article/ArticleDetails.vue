            ...
            <div>
                <el-card class="article-wrapper">
                    ...
                    <div class="aritcle-copyright">
                        <div>
                            <span>文章作者：</span>
                            <el-link type="info" href="/userinfo" target="_blank">UserInfo.name</el-link>
                        </div>
                        <div>
                            <span>文章链接：</span>
                    ...
                    <div class="article-reward">
                        ...
                        <strong>
                            <el-button style="width: 50px;height: 50px;margin: 0 15px 5px;" @click="like" plain circle>
                                <i style="font-size:20px" class="iconfont icon-shoucang"></i>
                            </el-button>
                            <span v-show="article.likeCount > 0">
                                {{article.likeCount}}
                            </span>
                    ...
                    <div class="recommend-div">
                        <div v-for="(item) in articleSimilarList" :key="item.articleId" class="recommend-item">
                            <div style="height: 24px;overflow: hidden;width: 100%;">
                                <a @click="toArticle(item.articleId)"><strong class="recommend-strong">{{item.title}}</strong></a>
                                <a @click="toUser(item.accountId)"><span class="rec-author">{{item.nickname}}的博客</span></a>
                            </div>
                            <div class="con-div">
                                <a @click="toArticle(item.articleId)"><span>{{item.summary}}</span></a>
                            </div>
                        </div>
...
import Comment from "../../front/article/components/Comment";
...
export default {
    ...
    methods: {
        ...
        like() {
            ...
            this.$post("/articles/like", param).then(res => {
                if (res.code === 0) {
                    //判断是否点赞
