<template>
    <div>
        <div class="serach">
            <el-form v-model="formData" :inline="true">
                ...
                <el-form-item label="标签ID">
                    <el-input @input="forceUpdate" type="text" v-model="formData.id" placeholder="请输入分类ID"
                        clearable />
                </el-form-item>

                <el-form-item label="标签名字">
                    <el-input @input="forceUpdate" type="text" v-model="formData.tagName" placeholder="请输入分类名字"
                        clearable />
                </el-form-item>
                <el-form-item label="标签描述">
                    <el-input @input="forceUpdate" type="text" v-model="formData.tagDesc" placeholder="请输入分类描述"
                        clearable />
                </el-form-item>
        ...
        <TagDialog :objectInfo="objectInfo" :centerDialogVisible.sync="centerDialogVisible"
...
export default {
    ...
    methods: {
        ...
        deleteObject(objectInfo) {
            MessageBox.confirm(`是否确认删除${objectInfo.tagName}?`, '温馨提示', {
                ...
                this.$delete('/tag/delete',{ids:objectInfo.id}).then(rs => {
                    Message.success(`删除${objectInfo.tagName}成功！`)
                    this.searchSubmit()
        ...
        addOrUpdateObject(type) {
            ...
            const service = Loading.service({
                text: `正在${actionType[type]}账号${this.objectInfo.tagName},请稍等...`,
                target: '.el-main'
            })
            if (type === 'add') {
                this.$post('/tag/insert',this.objectInfo).then(rs => {
                    Message.success('新增成功')
                    this.searchSubmit()
                }).finally(() => service.close())

            } else {
                this.$put('/tag/update',this.objectInfo).then(rs => {
        ...
        updateObjectInfo(e) {
            ...
            this.objectInfo.tagName = e.tagName;
            this.objectInfo.tagDesc = e.tagDesc;
            this.centerDialogVisible = true
        },
        addObjectInfo() {
            this.objectInfo.id = '';
            this.objectInfo.tagName = '';
            this.objectInfo.tagDesc = '';
            this.centerDialogVisible = true
        },
        getSelectIds(val) {
