...
<div>
    <div class="serach">
        <el-form v-model="formData" :inline="true">
            ...
            <el-form-item label="置顶ID">
                <el-input @input="forceUpdate" type="text" v-model="formData.id" placeholder="请输入置顶ID" clearable />
            </el-form-item>

            <el-form-item label="置顶文章ID">
                <el-input @input="forceUpdate" type="text" v-model="formData.articleId" placeholder="请输入置顶文章ID" clearable />
            </el-form-item>
    ...
    <div style="float: left;">
        <el-button @click="addObjectInfo">新增置顶</el-button>
    ...
    <FormDialog :objectInfo="objectInfo" :centerDialogVisible.sync="centerDialogVisible" @closeDialog="updateCenterDialogVisible" @success="addOrUpdateObject" />
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        deleteObject(objectInfo) {
            MessageBox.confirm(`是否确认删除${objectInfo.articleId}的置顶?`, '温馨提示', {
                ...
                this.$delete('/top/delete', {
                    ids: objectInfo.id
                }).then(rs => {
                    Message.success(`删除${objectInfo.articleId}成功！`)
                    this.searchSubmit()
        ...
        addOrUpdateObject(type) {
            ...
            const service = Loading.service({
                text: `正在${actionType[type]}账号${this.objectInfo.articleId},请稍等...`,
                target: '.el-main'
            })
            if (type === 'add') {
                this.$post('/top/insert', this.objectInfo).then(rs => {
                    Message.success('新增成功')
                    this.searchSubmit()
                }).finally(() => service.close())

            } else {
                this.$put('/top/update', this.objectInfo).then(rs => {
        ...
        updateObjectInfo(e) {
            ...
            this.objectInfo.articleId = e.articleId;
            this.centerDialogVisible = true
        },
        addObjectInfo() {
            this.objectInfo.id = '';
            this.objectInfo.articleId = '';
            this.centerDialogVisible = true
        },
        getSelectIds(val) {
        ...
        async batchDeleteBuild() {
            ...
            for (var i = 0; i < this.multipleSection.length; i++) {
                ...
                var name = this.multipleSection[j].articleId;
                this.deleteIds.push(id);
                this.deleteUsernames.push(name);
            }
            //删除deleteIds中的数据
            let ids = this.deleteIds
            this.$delete('/top/delete?' + ids.map(id => 'ids=' + id).join('&')).then(rs => {
