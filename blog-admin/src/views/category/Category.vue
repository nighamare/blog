<template>
    <div>
        <div class="serach">
            <el-form v-model="formData" :inline="true">
                ...
                <el-form-item label="分类ID">
                    <el-input @input="forceUpdate" type="text" v-model="formData.id" placeholder="请输入分类ID" clearable />
                </el-form-item>

                <el-form-item label="分类名字">
                    <el-input @input="forceUpdate" type="text" v-model="formData.categoryName" placeholder="请输入分类名字"
                        clearable />
                </el-form-item>
                <el-form-item label="分类描述">
                    <el-input @input="forceUpdate" type="text" v-model="formData.categoryDesc" placeholder="请输入分类描述"
                        clearable />
                </el-form-item>
        ...
        <div style="float: left;">
            <el-button @click="addObjectInfo">新增分类</el-button>
        ...
        <CategoryDialog :objectInfo="objectInfo" :centerDialogVisible.sync="centerDialogVisible"
...
export default {
    ...
    methods: {
        ...
        deleteObject(objectInfo) {
            MessageBox.confirm(`是否确认删除${objectInfo.categoryName}?`, '温馨提示', {
                ...
                this.$delete('/category/delete',{ids:objectInfo.id}).then(rs => {
                    Message.success(`删除${objectInfo.categoryName}成功！`)
                    this.searchSubmit()
        ...
        addOrUpdateObject(type) {
            ...
            const service = Loading.service({
                text: `正在${actionType[type]}账号${this.objectInfo.categoryName},请稍等...`,
                target: '.el-main'
            })
            if (type === 'add') {
                this.$post('/category/insert', this.objectInfo).then(rs => {
                    Message.success('新增成功')
                    this.searchSubmit()
                }).finally(() => service.close())

            } else {
                this.$put('/category/update', this.objectInfo).then(rs => {
        ...
        updateObjectInfo(e) {
            ...
            this.objectInfo.categoryName = e.categoryName;
            this.objectInfo.categoryDesc = e.categoryDesc;
            this.centerDialogVisible = true
        },
        addObjectInfo() {
            this.objectInfo.id = '';
            this.objectInfo.categoryName = '';
            this.objectInfo.categoryDesc = '';
            this.centerDialogVisible = true
        },
        getSelectIds(val) {
